...
async def seed_challenges():
    ...
    # Clean up previous attempts to allow clean re-runs
    print("Cleaning up old seed data if present...")
    await conn.execute("""
        DELETE FROM core.problem_solutions 
        WHERE problem_id IN (SELECT id FROM core.problems WHERE title IN ('Python Salary Bonus', 'PySpark High Salary Count'))
    """)
    await conn.execute("""
        DELETE FROM core.problem_datasets 
        WHERE problem_id IN (SELECT id FROM core.problems WHERE title IN ('Python Salary Bonus', 'PySpark High Salary Count'))
    """)
    await conn.execute("""
        DELETE FROM core.problems 
        WHERE title IN ('Python Salary Bonus', 'PySpark High Salary Count')
    """)
